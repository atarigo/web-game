...
export function createPlayer(tex: Record<SurvivalSpriteKey, Texture>) {
  ...
  pLL.rect(-6, 8, 5, 13).fill(0x4a4a6a);
  const pRL = new Graphics();
  pRL.rect(1, 8, 5, 13).fill(0x4a4a6a);
  pBody.addChild(pLL, pRL);
  const pTorso = new Graphics();
  pTorso.rect(-7, -10, 14, 20).fill(0x3a6aaa);
  pBody.addChild(pTorso);
  const pHead = new Graphics();
  pHead.circle(0, -19, 9).fill(0xdda070);
  pBody.addChild(pHead);
  const pEye = new Graphics();
  pEye.circle(3, -20, 2).fill(0x222222);
  pBody.addChild(pEye);
      ...
      : action === "walk"
        ...
        : action === "work"
          ...
          : action === "chop"
            ...
            : `Attack${frame}`;
  return tex[
    `hero${genderCap}${dir}${suffix}` as SurvivalSpriteKey
...
export function updateCarryVisual(P: Player, pCarryC: Container) {
  ...
  for (let i = 0; i < parts.length; i++) {
    const t = centeredText(
      parts[i],
      -totalW / 2 + spacing / 2 + i * spacing,
      -68,
      { size: 10, color: COLORS.yellow, family: "Arial" },
    );
    pCarryC.addChild(t);
  }
}
...
export function updatePlayerVisual(
  ...
  if (
    atkRange &&
    P.tgt &&
    P.atkT > ACD - ("vx" in P.tgt ? HERO_ATTACK_ANIM_MS : HERO_CHOP_ANIM_MS)
  ) {
    ...
    pSprite.texture = heroTexture(
      tex,
      P,
      gender,
      isBearTarget ? "attack" : "chop",
      frame,
    );
    // 新幀為 256 大畫布：以等比縮放取代舊的 width/height 設定，畫面高度不變（66/58）
    pSprite.scale.set(HERO_ATTACK_SCALE);
  } else if (moving) {
    const frame = Math.floor(P.walkT / 8) % 2 === 0 ? 1 : 2;
    pSprite.texture = heroTexture(tex, P, gender, "walk", frame);
    pSprite.scale.set(HERO_SCALE);
  } else {
  ...
  hpBar(
    ...
    0,
    -58,
    P.hp > P.maxHp * 0.3 ? COLORS.green : COLORS.red,
  );
