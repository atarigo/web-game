...
import { spriteMeta, type SurvivalSpriteKey } from "./assets";
import { WW, WH, FOREST, HUNT, WSTN, MSTN, BENTO } from "./constants";

export function makeSprite(
  tex: Record<SurvivalSpriteKey, Texture>,
  key: SurvivalSpriteKey,
  width: number,
  height: number,
  anchorY = 0.82,
) {
  const s = new Sprite(tex[key]);
  // 角色類為 256 大畫布新幀：改用 ooxx 錨點與等比縮放，讓畫面高度與舊版一致
  const meta = spriteMeta(key);
  if (meta) {
    s.anchor.set(meta.anchorX, meta.anchorY);
    s.scale.set(meta.scale);
  } else {
...
export function buildGround(
  ...
  groundFallback
    .circle(FOREST.x, FOREST.y, FOREST.r + 16)
    .fill({ color: 0x263f38, alpha: 0.75 });
  groundFallback
    .circle(HUNT.x, HUNT.y, HUNT.r + 16)
    .fill({ color: 0x4a3a2c, alpha: 0.65 });
  groundFallback
    .moveTo(40, 360)
    .bezierCurveTo(120, 330, 300, 330, 380, 360)
    .stroke({ color: 0xd9eef0, width: 18, alpha: 0.12 });
  groundFallback
    .moveTo(65, 520)
    .bezierCurveTo(145, 470, 280, 470, 355, 515)
    .stroke({ color: 0xd9eef0, width: 22, alpha: 0.12 });
...
export function buildZones(
  ...
  function makeStn(
    ...
    base
      .roundRect(-30, -23, 60, 42, 5)
      .stroke({ color: 0x7f8f95, width: 1, alpha: 0.45 });
    base.visible = false;
    c.addChild(base);
  ...
  zoneLayer.addChild(
    centeredText("森林", FOREST.x, FOREST.y - FOREST.r - 12, {
      size: 11,
      color: COLORS.muted,
      family: "Arial",
    }),
  ...
  zoneLayer.addChild(
    centeredText("獵場", HUNT.x, HUNT.y - HUNT.r - 12, {
      size: 11,
      color: COLORS.muted,
      family: "Arial",
    }),
...
export function buildStockTexts(zoneLayer: Container): StockTexts {
  const wStockT = centeredText("", WSTN.x, WSTN.y - 28, {
    size: 11,
    color: COLORS.yellow,
    family: "Arial",
  });
  const mStockT = centeredText("", MSTN.x, MSTN.y - 28, {
    size: 11,
    color: COLORS.yellow,
    family: "Arial",
  });
  const wLvT = centeredText("Lv1", WSTN.x, WSTN.y - 40, {
    size: 10,
    color: COLORS.cyan,
    family: "Arial",
  });
  const mLvT = centeredText("Lv1", MSTN.x, MSTN.y - 40, {
    size: 10,
    color: COLORS.cyan,
    family: "Arial",
  });
  const bStockT = centeredText("", BENTO.x - 20, BENTO.y - 38, {
    size: 11,
    color: COLORS.yellow,
    family: "Arial",
  });
  const bCoinT = centeredText("", BENTO.x + 20, BENTO.y - 38, {
    size: 11,
    color: COLORS.gold,
    family: "Arial",
  });
