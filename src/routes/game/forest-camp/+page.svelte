<script lang="ts">
	...
	import { loadForestCampTextures, heroPreviewUrls } from './assets';
	...
	async function startGame(mode: 'normal' | 'infinite') {
		...
		const tex = await loadForestCampTextures(app.renderer, selectedGender);
