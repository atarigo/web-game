...
export function hpBar(g: Graphics, hp: number, max: number, ox: number, oy: number, col: number) {
	g.clear();
	const bw = 32;
...
export function popup(s: string, x: number, y: number, col: number, fxLayer: Container, app: Application) {
...
export function spawnDrop(type: IType | 'coin', x: number, y: number, itemLayer: Container, drops: Drop[]) {
...
export function flyTo(d: Drop, tx: number, ty: number, cb: () => void, app: Application) {
	d.flying = true;
	const fx = d.x, fy = d.y;
	let t = 0;
	const tk = function () {
...
export function ucost(lv: number) { return Math.floor(8 * Math.pow(1.6, lv - 1)); }
export function prodMs(lv: number) { return 1000 / (1 + (lv - 1) * 0.35); }
