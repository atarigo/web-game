...
import { FOREST, HUNT, DMG, AGGRO_R, ATK_R, BEAR_SPD, BEAR_DMG, BEAR_ACD } from './constants';
...
export function hitTree(
	...
	if (t.hp <= 0) {
		...
		stats.statTrees++; stats.score += 10;
		const n = 2 + Math.floor(Math.random() * 2);
		for (let i = 0; i < n; i++) spawnDrop('wood', t.x, t.y, itemLayer, drops);
	} else {
		if (t.hp <= t.max / 2) t.sprite.texture = tex.treeDamaged as unknown as Texture;
...
export function hitBeast(
	...
	if (b.hp <= 0) {
		...
		stats.statBeasts++; stats.score += 10;
		const n = 2 + Math.floor(Math.random() * 2);
	...
	for (let i = 0; i < beasts.length; i++) {
		...
		if (hasAggro) {
			...
			if (dd <= ATK_R && a.atkCD <= 0) {
				a.atkCD = BEAR_ACD;
				a.attackT = 420;
				if (aggroIsPlayer) hitPlayerByBeast(BEAR_DMG, P, fxLayer, app, onPlayerDie);
				else if (aggroWk) hitHunterByBeast(aggroWk, BEAR_DMG, allWk, hunterWks, fxLayer, app);
			}
			a.walkT += tf;
		} else {
		...
		if (a.hitT > 0) {
			...
			a.sprite.texture = tex[a.attackT > 210 ? 'bearAttack1' : 'bearAttack2'] as unknown as Texture;
		} else {
			a.sprite.texture = tex[Math.floor(a.walkT / 18) % 2 === 0 ? 'boarWalk1' : 'boarWalk2'] as unknown as Texture;
		}
