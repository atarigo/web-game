...
export function spawnWk(
	...
	const body = new Graphics(); body.rect(-5, -5, 10, 14).fill(kind === 'lumber' ? 0x466548 : kind === 'hunter' ? 0x65543f : 0x5a5f63); c.addChild(body);
	...
	const sprite = makeSprite(tex, (kind === 'lumber' ? 'lumberDownIdle' : kind === 'hunter' ? 'hunterDownIdle' : `${clerkPrefix(clerkRole)}DownIdle`) as SurvivalSpriteKey, 36, 46, 0.86);
	c.addChild(sprite);
...
export function setWorkerTexture(wk: Wk, tex: Record<SurvivalSpriteKey, Texture>, timer: number) {
	...
	if (wk.hunger === 'working' && wk.walkT === 0) {
		...
		wk.sprite.texture = tex[`${prefix}${dir}Work${workFrame}` as SurvivalSpriteKey] as unknown as Texture;
		return;
	}
	if (wk.walkT > 0) {
		wk.sprite.texture = tex[`${prefix}${dir}Walk${Math.floor(wk.walkT / 10) % 2 === 0 ? 1 : 2}` as SurvivalSpriteKey] as unknown as Texture;
		return;
	}
	wk.sprite.texture = tex[`${prefix}${dir}Idle` as SurvivalSpriteKey] as unknown as Texture;
}
...
export function updateWorkersAI(
	...
	app: Application,
	tex: Record<SurvivalSpriteKey, Texture>,
	gs: { bentoStock: number; score: number; statTrees: number; statBeasts: number },
	timer: number,
	dt: number,
	tf: number,
	refreshUI: () => void
				...
				if (wk.tgt) {
					...
					if (d2 > 25) {
						...
						if (wk.atkT <= 0) {
							...
							hitTree(wk.tgt as Tree, trees, stumps, objLayer, tex, drops, itemLayer, fxLayer, app, gs);
				...
				if (wk.tgt) {
					...
					if (d2 > 25) {
						...
						if (wk.atkT <= 0) {
							...
							hitBeast(wk.tgt as Beast, beasts, drops, itemLayer, fxLayer, app, tex, gs);
