...
export function refreshUI(
	...
	const sec = mode === 'normal' ? Math.max(0, Math.ceil(gs.timer / 1000)) : Math.floor(gs.timer / 1000);
...
export function mkBtn(
	...
	c.cursor = 'pointer';
	function doTrigger() {
		if (isOn()) { inp.btnTapped = true; onClick(); doRefresh(); }
	}
...
export function createButtons(
	...
	function hasBentoStock() { return gs.bentoStock > 0; }
	function useBentoStock() { if (gs.bentoStock > 0) gs.bentoStock--; }
	...
	mkBtn(
		...
		function () { return pNearW() && !wkRefs.wStnWk; },
		function () { return hasBentoStock(); },
		function () { useBentoStock(); wkRefs.wStnWk = spawnWk(WSTN.x + 25, WSTN.y, 'clerk', 'wood', tex, charLayer, allWk); wkRefs.wStnWk.homeX = WSTN.x + 25; wkRefs.wStnWk.homeY = WSTN.y; popup('+👷', WSTN.x, WSTN.y - 30, COLORS.green, fxLayer, app); },
		H - 55, hudLayer, btns, inp, doRefresh
	);
	mkBtn(
		function () { return '雇用樵夫 🍱1'; },
		function () { return pNearW() && !wkRefs.cutterWk; },
		function () { return hasBentoStock(); },
		function () { useBentoStock(); wkRefs.cutterWk = spawnWk(FOREST.x, FOREST.y + 30, 'lumber', 'bento', tex, charLayer, allWk); wkRefs.cutterWk.homeX = FOREST.x; wkRefs.cutterWk.homeY = FOREST.y + 30; popup('+🪓', WSTN.x, WSTN.y - 30, COLORS.green, fxLayer, app); },
		H - 95, hudLayer, btns, inp, doRefresh
	);
	mkBtn(
		function () { return gs.wLv >= MAX_LV ? '木材站 MAX' : '⬆木材站Lv' + (gs.wLv + 1) + ' 💰' + ucost(gs.wLv); },
		function () { return pNearW() && gs.wLv < MAX_LV; },
		function () { return carryCoins() >= ucost(gs.wLv); },
		function () { spendCoins(ucost(gs.wLv)); gs.wLv++; popup('⬆Lv' + gs.wLv, WSTN.x, WSTN.y - 40, COLORS.cyan, fxLayer, app); },
		H - 135, hudLayer, btns, inp, doRefresh
	);

	mkBtn(
		function () { return '雇用店員 🍱1'; },
		function () { return pNearM() && !wkRefs.mStnWk; },
		function () { return hasBentoStock(); },
		function () { useBentoStock(); wkRefs.mStnWk = spawnWk(MSTN.x + 25, MSTN.y, 'clerk', 'meat', tex, charLayer, allWk); wkRefs.mStnWk.homeX = MSTN.x + 25; wkRefs.mStnWk.homeY = MSTN.y; popup('+👷', MSTN.x, MSTN.y - 30, COLORS.green, fxLayer, app); },
		H - 55, hudLayer, btns, inp, doRefresh
	);
	mkBtn(
		function () { return '雇用獵人 🍱1'; },
		function () { return pNearM() && wkRefs.hunterWks.length < 2; },
		function () { return hasBentoStock(); },
		function () { useBentoStock(); const ox = wkRefs.hunterWks.length === 0 ? 0 : 20; const hw = spawnWk(HUNT.x + ox, HUNT.y + 30, 'hunter', 'bento', tex, charLayer, allWk); hw.homeX = HUNT.x + ox; hw.homeY = HUNT.y + 30; wkRefs.hunterWks.push(hw); popup('+🏹', MSTN.x, MSTN.y - 30, COLORS.green, fxLayer, app); },
		H - 95, hudLayer, btns, inp, doRefresh
	);
	mkBtn(
		function () { return gs.mLv >= MAX_LV ? '肉品站 MAX' : '⬆肉品站Lv' + (gs.mLv + 1) + ' 💰' + ucost(gs.mLv); },
		function () { return pNearM() && gs.mLv < MAX_LV; },
		function () { return carryCoins() >= ucost(gs.mLv); },
		function () { spendCoins(ucost(gs.mLv)); gs.mLv++; popup('⬆Lv' + gs.mLv, MSTN.x, MSTN.y - 40, COLORS.cyan, fxLayer, app); },
		H - 135, hudLayer, btns, inp, doRefresh
	);
	...
	mkBtn(
		...
		function () { useBentoStock(); P.hp = P.maxHp; popup('+❤️', P.x, P.y - 65, COLORS.green, fxLayer, app); },
		H - 55, hudLayer, btns, inp, doRefresh
	);
	mkBtn(
		function () { return '雇用店員 🍱1'; },
		function () { return pNearB() && !wkRefs.bentoWk; },
		function () { return hasBentoStock(); },
		function () { useBentoStock(); wkRefs.bentoWk = spawnWk(BENTO.x + 25, BENTO.y, 'clerk', 'bento', tex, charLayer, allWk); wkRefs.bentoWk.homeX = BENTO.x + 25; wkRefs.bentoWk.homeY = BENTO.y; popup('+👷', BENTO.x, BENTO.y - 30, COLORS.green, fxLayer, app); },
		H - 95, hudLayer, btns, inp, doRefresh
	);
}
