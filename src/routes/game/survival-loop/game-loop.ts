...
export function startGameLoop(
  ...
  app.ticker.add(function () {
    ...
    if (
      inp.joyActive &&
      (Math.abs(inp.joyDx) > 0.05 || Math.abs(inp.joyDy) > 0.05)
    ) {
      inputDx = inp.joyDx;
      inputDy = inp.joyDy;
      directInput = true;
    ...
    if (directInput) {
      ...
      for (let i = 0; i < trees.length; i++) {
        if (dist(P.x, P.y, trees[i].x, trees[i].y) < 30) {
          P.tgt = trees[i];
          break;
        }
      }
      if (!P.tgt) {
        for (let i = 0; i < beasts.length; i++) {
          if (dist(P.x, P.y, beasts[i].x, beasts[i].y) < 30) {
            P.tgt = beasts[i];
            break;
          }
        }
    ...
    P.atkT -= dt;
    const atkRange =
      P.tgt && !P.tgt.c.destroyed && dist(P.x, P.y, P.tgt.x, P.tgt.y) < 30;
    ...
    if (gs.pickupCD <= 0) {
      for (let i = 0; i < drops.length; i++) {
        ...
        const typeCount = P.carry.filter(function (c) {
          return c === dd.type;
        }).length;
    ...
    const wStnActive =
      (wkRefs.wStnWk && wkRefs.wStnWk.hunger === "working") ||
      dist(P.x, P.y, WSTN.x, WSTN.y) < DR;
    ...
    const mStnActive =
      (wkRefs.mStnWk && wkRefs.mStnWk.hunger === "working") ||
      dist(P.x, P.y, MSTN.x, MSTN.y) < DR;
    ...
    const bentoActive =
      (wkRefs.bentoWk && wkRefs.bentoWk.hunger === "working") ||
      dist(P.x, P.y, BENTO.x, BENTO.y) < DR;
      ...
      if (gs.bentoProdT >= 1200) {
        ...
        if (wkRefs.bentoWk && wkRefs.bentoWk.hunger === "working") {
          ...
          if (wkRefs.bentoWk.workCount >= 5) {
            if (gs.bentoStock > 0) {
              gs.bentoStock--;
              wkRefs.bentoWk.workCount = 0;
            } else {
              wkRefs.bentoWk.hunger = "eating";
            }
          }
        }
        refreshUI();
    ...
    gs.treeSpT += dt;
    if (gs.treeSpT > 2500 && trees.length < 5) {
      gs.treeSpT = 0;
      spawnTree(trees, stumps, layers.objLayer, tex);
    }
    gs.beastSpT += dt;
    if (gs.beastSpT > 1000 && beasts.length < 8) {
      gs.beastSpT = 0;
      spawnBeast(beasts, layers.objLayer, tex);
    }
